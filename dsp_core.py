...
def procesar_ecualizador(x, fs, ganancias_db):
    ...
    Usa secciones de segundo orden (SOS) para evitar inestabilidad en bajas frecuencias.
    """
    bandas = [
        (16, 60),       # Sub-Bass
        (60, 250),      # Bass
    ...
    # Creamos un arreglo de ceros del mismo tamano que x para ir sumando las bandas
    y_eq = np.zeros_like(x, dtype=np.float64)
    
    for i in range(len(bandas)):
        f_low, f_high = bandas[i]
        
        # Pasamos la ganancia de decibelios a escala lineal para poder multiplicar la señal
        ...
        # Scipy pide las frecuencias normalizadas de 0 a 1 (donde 1 es la frecuencia de Nyquist)
        Wn = [f_low / nyquist, f_high / nyquist]
        
        # Ojo aqui para la defensa: Usamos 'sos' (Second-Order Sections) en vez de [b, a].
        # Si usamos b, a, los polos se agrupan en z=1 para las frecuencias muy bajas (como 16 Hz) 
        # y el filtro explota arrojando ceros por cancelacion catastrofica de decimales.
        sos = sig.butter(4, Wn, btype='bandpass', output='sos')
        
        # filtfilt pasa el filtro hacia adelante y hacia atras. 
        # Esto nos garantiza fase cero y evita que el audio se distorsione en el tiempo.
        y_filtrado = sig.sosfiltfilt(sos, x)
        
        # Sumamos la contribucion de esta banda especifica
...
def generar_graficas_tiempo(x, y, z, modo="micro"):
    ...
    if modo == "Vista Micro (Muestras discretas stem)":
        # Extraemos solo las primeras 100 muestras para que los tallos (stem) sean visibles
        n_x = np.arange(min(100, len(x)))
        n_y = np.arange(min(100, len(y)))
        n_z = np.arange(min(100, len(z)))
        ...
        ax1.set_title("x[n]: Entrada Original (Primeras 100 muestras)")
        ax2.set_title("y[n]: Tras Cambio de Tasa L/M (Primeras 100 muestras)")
        ax3.set_title("z[n]: Tras Ecualizacion (Primeras 100 muestras)")
    else:
